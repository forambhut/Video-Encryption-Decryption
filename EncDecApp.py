...
class EncryptionTool:
    def __init__(self, user_file, user_key, user_salt):
        ...
        self.encrypt_output_file = ".".join(self.user_file.split(".")[:-1]) \
              + "." + self.file_extension + "_encrypt." + self.file_extension

        # decrypted file name
        self.decrypt_output_file = self.user_file[:-12].split(".")
...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        root.title("Video Encryption Decryption App")
    ...
    def show_help_callback(self):
        messagebox.showinfo(
            "How To",
            """1. Open the App and Click SELECT FILE Button and select your file e.g. "abc.mp4".
2. Enter your Secret Key (This can be any alphanumeric letters). Remember this so you can Decrypt the file later.
3. Click ENCRYPT Button to encrypt. A new encrypted file with "_encrypt.mp4" extention e.g. "abc.mp4_encrypt" will be created in the same directory where the "abc.mp4" is.
4. When you want to Decrypt a file you, will select the file with the "_encrypt.mp4" extention and Enter your Secret Key which you chose at the time of Encryption. Click DECRYPT Button to decrypt. The decrypted file will be of the same name as before with the suffix "__decrypted__" e.g. "abc__decrypted__.mp4".
